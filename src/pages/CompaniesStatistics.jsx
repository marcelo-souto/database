...
import styles from './CompaniesStatistics.module.css';
      ...
      <div className={styles.bar}>
        {data &&
          data.map((comp, index) => (
            <Bar
              key={comp.name + index}
              onClick={() => getGames(comp.id, comp.name, comp.games)}
              company={comp}
            />
          ))}
      </div>
      ...
      {companyGames && (
        <div className={`${styles.games} comeFromBottom`}>
          <h1 className={styles.title}>
            Principais jogos:{' '}
            <span className={styles.titleName}>{compName}</span>
          </h1>
          {companyGames.map((game, index) => (
            <div>
              <p className={styles.game}>
                <span className={styles.placing}>{index + 1}°</span> -{' '}
                {game.name} -
                <span className={styles.note}>{game.meta_score}/100</span>
              </p>
            </div>
          ))}
          ...
          <p className={styles.total}>
            Total de jogos :{' '}
            <span className={styles.note}>{compTotal.length}</span>
          </p>
        </div>
      )}
    </div>
